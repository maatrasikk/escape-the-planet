...
const SHIELD_REPAIR_PER_ENERGY: int = 10
...
func get_daily_spend_left() -> int:
	var used := alloc_search + alloc_antenna + alloc_shield
	return max(0, get_daily_limit() - used)

func set_allocations(search: int, antenna: int, shield: int = 0) -> void:
	search = max(search, 0)
	antenna = max(antenna, 0)
	shield = max(shield, 0)
	...
	if total > limit:
		var scale := float(limit) / float(total)
		search = int(floor(search * scale))
		antenna = int(floor(antenna * scale))
		shield = int(floor(shield * scale))

	while search + antenna + shield < limit:
		if search <= antenna and search <= shield:
			search += 1
		elif antenna <= shield:
			antenna += 1
		else:
...
func next_day() -> void:
# 1. Применяем траты энергии (в пределах доступного лимита)
	var total_alloc := alloc_search + alloc_antenna + alloc_shield
	var limit := get_daily_limit()
	total_alloc = min(total_alloc, limit)
	energy -= total_alloc
	if energy < 0:
		energy = 0  # на всякий случай
	# 2. Поиск обломка (максимум один)
	...
	# 5. Если враги присутствовали — наносят урон сейчас
	last_enemy_damage = 0
	if enemy_present:
	...
	# 6. Проверка победы / поражения после всех эффектов текущего дня
	...
	# 7. Генерируем врагов на следующий день (если игра не окончена)
	if not victory and not defeat:
		enemy_present = (_rng.randf() < ENEMY_SPAWN_CHANCE)
	else:
		enemy_present = false  # останавливаем спавн
