...
func open_at_world(world_pos: Vector2) -> void:
	...
	if cam:
		...
		var screen := (world_pos - cam.global_position) * cam.zoom + screen_center
...
func _on_slider_changed(kind: String, val: float) -> void:
	if _updating_sliders:
		...
		var limit := GameState.get_daily_limit()

		var s := int(search_slider.value)
		var a := int(antenna_slider.value)
		var sh := int(shield_slider.value)
		...
		# Если перебор — уменьшаем последний изменённый
		var total := s + a + sh
		if total > limit:
		...
		# Если ресурсы или цели достигнуты — корректируем
...
func _update_alloc_labels() -> void:
	var left := GameState.get_daily_allocation_left()
	...
	# Кнопку можно нажимать всегда (нет требования тратить всё)
	next_day_button.disabled = (_game_over or GameState.energy <= 0)

func _refresh_all() -> void:
	var st := GameState.get_status_dict()
	...
	# Ограничения на слайдеры по текущему лимиту
	var limit := GameState.get_daily_limit()
	for s in [search_slider, antenna_slider, shield_slider]:
		s.max_value = limit

	# Блокируем слайдеры если цель достигнута
	...
	# Ставим значения (распределения могли быть сброшены)
...
func _on_game_over(victory, reason):
	_game_over = true
	next_day_button.disabled = true
